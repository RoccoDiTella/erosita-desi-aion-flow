...
def test_shared_head_shape() -> None:
    head = SharedCLSHead()
    out = head(torch.randn(3, N_TARGETS, 768))
    assert out.shape == (3, N_TARGETS, 256)
...
def test_masked_loss_skips_nan_targets_and_flows_gradients() -> None:
    ...
    contexts = torch.randn(B, N_TARGETS, 256, requires_grad=True)
    ...
    total, raw = multi_target_nll(
        ...
        standardizers=stds, weights=np.ones(N_TARGETS), inject=False,
    )
...
def test_injection_only_where_sigma_present() -> None:
    ...
    contexts = torch.zeros(B, N_TARGETS, 256)
    stds = [TargetStandardizer(0.0, 1.0) for _ in range(N_TARGETS)]
