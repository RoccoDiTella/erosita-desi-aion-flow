...
Survey imaging, WISE mid-infrared photometry, and redshift — and measures *how much* X-ray
information each optical/IR modality carries.
...
<p align="center">
  <img src="assets/architecture.png" width="760" alt="Model architecture: frozen AION encoder, attention-pooling head, conditional neural spline flow.">
</p>

**Paper.** *Exploring the connection between optical and X-ray emissions in Active Galactic Nuclei* —
Rebeka L. Bottger\*, Rocco Di Tella\*, Carolina Cuesta-Lazaro, Jason Reeves, Nico Cappelluti, and
Rafael Martínez-Galarza &nbsp;(\*equal contribution). Conference on Physics and AI (PAI 2026),
Stanford University. Affiliations: Center for Astrophysics | Harvard & Smithsonian · AstroAI ·
University of Miami · Flatiron Institute · Institute for Advanced Study · University of Illinois
Urbana-Champaign.

**Code.** This repository was written and is maintained by **Rocco Di Tella**, who is solely
responsible for any errors in it. The other authors listed above are authors of the paper.

---

## Overview

Optical light traces large-scale galaxy properties (stellar populations, structure, gas), while
X-rays probe accretion in the immediate vicinity of the black hole. Despite this scale separation,
the two are empirically linked. We frame the link as a question of **shared information**: given a
frozen multimodal embedding of the optical/IR view of a source, how well — and how *confidently* —
can we predict its X-ray flux?

Rather than a point estimate, the model outputs a **full conditional density** `p(log F_X | inputs)`
via a normalizing flow. This lets us report calibrated likelihood-based metrics:
...
X-ray information*. Across all four modalities the model reaches R² = 0.549 and a likelihood-gain
factor exp(IG) = 1.405 on held-out sources — well above a classical emission-line baseline (R² = 0.392).
...
spectra are present buys nothing measurable. We report **all four inputs** as the headline — it's the
full multimodal model — rather than the marginal R² winner. Either way the flow comfortably beats the
...
Information gain is not a fixed number — it varies with redshift, with spectra and imaging dominating
at low redshift and WISE/redshift becoming more informative at high redshift:
...
```text
DESI spectrum · redshift · WISE W1/W2/W3 · Legacy Survey griz image
        │   native AION modalities; missing inputs are simply omitted (no fake masking)
        ▼
frozen AION-base encoder  ──►  per-modality token sequence  [B, T, 768]
        ▼
modality-affine calibration
   → 4 learned global queries + 16-way modality-presence embedding
   → 2 attention-pooling blocks (8 heads: cross-attention + self-attention + FFN)
   → flatten 4 × 768 = 3072  →  MLP  3072 → 512 → 512 → 256
        ▼
conditional Zuko neural spline flow  ──►  p(log X-ray flux | inputs)
```
...
input list rather than zero-masking it. The learned queries are offset by a modality-presence
embedding (one of 2⁴ states), which lets the pooling adapt to whichever inputs are actually present.
...
Full training/evaluation needs both `zuko` and `polymathic-aion`. The unit tests run with only the
core install.
...
A fast staging smoke test (no AION/zuko required) runs on a handful of sources:
...
attention-pooling blocks with eight heads, a one-dimensional neural spline flow (8 transforms, 256-d
context). The training recipe here is intentionally **simplified** (a uniform modality-combination
sampler in place of the paper's tuned mixture), so it is meant to *reach the neighborhood* of the
paper's Table 1 (all-inputs R² ≈ 0.55, exp(IG) ≈ 1.40) rather than reproduce it bit-for-bit.
...
[MIT](LICENSE) © 2026 Rocco Di Tella.
