...
- ▶ **Running now (order changed by decision: light-first, reproduction deferred):** V1 head + cleaned view + `--error-mode convolve`, `log_ml_flux_1`, 15 epochs, batch 448, A100-80 (`gpu`, job 34083239) — the first results-grade run of the new-science config.
- ⏳ Then: **paper-table reproduction** (full q4/l2 head, `--error-mode none`, native paper splits, 50 epochs) → compare to published 0.549/0.554; consider the AION **token cache** (~185 GB per-combo, ~5–15× epoch speedup) before the 4-target production sweep; `siag_gpu` once siag_lab membership is granted.
