...
## 2. Compute
- **FASRC** (Harvard SLURM). Account `siag_lab`; partition `siag_gpu` (32× A100); smokes on `gpu_test`.
- Env: `module load Miniforge3/26.1.0-fasrc01` + conda env + **cu124 torch wheel** + `pip install -e . polymathic-aion[torch] zuko safetensors`.
- Non-interactive access via SSH ControlMaster socket `~/.ssh/cm-fasrc`.
...
- Four separate V1 runs (`--target {log_ml_flux_1,log_lx,logmstar,hr32}`), `--clean` (NWAY keep), `--error-mode convolve`, on `siag_gpu`; smoke on `gpu_test` with a `--limit` subset.
...
- ⏳ On FASRC: data staging ~76% (to `/n/netscratch/finkbeiner_lab/Lab/rditella/aionflow`) → then code+env, `prepare-data --clean`, and the V1 `log_ml_flux_1` smoke (`--error-mode convolve`) on `gpu_test`.
- ⚠️ `siag_lab` membership pending (Coldfront) → real `siag_gpu` runs after; `gpu_test` meanwhile.
