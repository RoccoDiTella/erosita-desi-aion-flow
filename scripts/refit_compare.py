...
def main() -> None:
    ...
    eps = sorted(bodies)
    heads = sorted({n for b in bodies.values() for n in b})
    ...
    tot = {e: sum(r["val_nll"] for r in bodies[e].values()) for e in fin_bodies}
    ...
    for h in sorted(bodies[pick], key=lambda n: bodies[pick][n]["best_epoch"]):
        r = bodies[pick][h]
    ...
    print(f"{'body':>6s} {'joint':>9s} {'sum marg':>10s} {'dependence':>12s}")
    for e in fin_bodies:
        have = [bodies[e][d]["val_nll"] for d in joint_dims if d in bodies[e]]
        if len(have) != len(joint_dims):
            continue
        j = joint_by_epoch.get(e)
        if j is None:
            continue
        print(f"{('ep' + str(e)):>6s} {j:>9.3f} {sum(have):>10.3f} {sum(have) - j:>+12.3f}")
